<?php
require 'inc/head_foot.inc';

$title = "Papers";
phead();
?>

<h2 style="text-align:center;">Papers</h2>

<ul>
  <li>
    Hull, C. L. H., Plambeck, R. L., et al., <i> Misalignment of Magnetic 
    Fields and Outflows in Protostellar Cores</i>, 2013, ApJL, (submitted)
  </li>

  <li>
    Hughes, A. M., Hull, C. L. H., Wilner, D. J., Plambeck, R. L.,
    <i>Interferometric Upper Limits on Millimeter Polarization of the Disks 
    around DG Tau, GM Aur, and MWC 480</i>, 2013, AJ, (submitted)
  </li>

  <li>
    Krumholz, M. R., Crutcher, R. M., Hull, C. L. H., <i>Protostellar Disk
    Formation Enabled by Weak, Misaligned Magnetic Fields</i>, 2013, ApJL,
    (submitted)
  </li>

  <li>
    Stephens, I., Looney, L., Hull, C., Plambeck, R., Kwon, W., Chapman, R., 
    Novak, G., Matthews, T., Vaillancourt, J., Shinnaga, H., Davidson, J., and
    Crutcher, R., 
    <i>The Magnetic Field Morphology of the Class 0 Protostar L1157</i>, 2012,
    (in prep)
  </li>
</ul>

<div class="clear"></div>

<?php pfoot();?>
